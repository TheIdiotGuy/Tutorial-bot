...
module.exports = {
	...
	async run(client, interaction) {
		...
		if (!fetchUser) return interaction.reply({ content: `You need to mention someone to Slap!`, ephemeral: true });

		const { body } = await superagent
			.get("https://nekos.life/api/slap");

		const slap = new Discord.MessageEmbed()
			.setTitle(`${interaction.user.username} Slaps ${fetchUser.username}`)
			.setImage(body.url)
			.setColor("#5440cd")
